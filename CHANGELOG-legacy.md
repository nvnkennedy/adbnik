...
# Adbnik Changelog (archived body)

All notable changes to this project were documented here before the **1.0.0** reset.
...
- **License:** Switched to **Apache License 2.0** (SPDX `Apache-2.0`) — permissive, widely accepted in enterprise reviews; includes explicit **patent grant** and structured **notice**/`NOTICE` obligations for redistributions. Replaced the prior **0BSD** license text with the canonical [`LICENSE`](LICENSE) from apache.org; added [`NOTICE`](NOTICE).

## [5.1.1] - 2026-05-03

### Changed

- **License:** Replaced the prior MIT-style text with **0BSD (BSD Zero Clause)** — same idea (very permissive, no obligation to copy a copyright notice into downstream releases) but it is **not** the “MIT” license family; SPDX `0BSD` (see [`LICENSE`](LICENSE)).

## [5.1.0] - 2026-05-03

### Changed

- **License:** Switched from MIT to **MIT No Attribution (MIT-0)** — use and redistribute without being required to include a copyright notice in your distributions (see [`LICENSE`](LICENSE)). *(Superseded by **5.1.1**: project now uses **0BSD**.)*

### Added

- **Windows packaging:** PyInstaller **onedir** spec (`packaging/windows/adbnik.spec`), Inno Setup **unsigned** installer script (`packaging/windows/adbnik.iss`), `build.ps1`, and GitHub Actions workflow **`windows-installer`** (artifacts: portable zip + `Adbnik-*-Setup-unsigned.exe`). README documents **SmartScreen / “Run anyway”** for the unsigned installer and clarifies the warning is due to **missing code signing**, not malware detection.
